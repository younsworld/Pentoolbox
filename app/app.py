...
# app.py vit dans app/ (cf. restructuration) — templates/ et
...
# du code et du reste du projet — voir la doc projet pour le detail de la
...
# cf. doc "Running it"), il n'y a pas de proxy TLS devant Flask — passer
...
# Identifiants par defaut (bootstrap, cf. load_users). Si un compte se connecte
# ENCORE avec son mot de passe par defaut, on force le changement au 1er login
# (cf. BUG 5 - modal non-fermable cote frontend). Detection sans etat : une fois
# le mdp change, il ne correspond plus a cette table -> plus de modal. C'est le
# changement de hash lui-meme qui "marque" le 1er login comme termine.
DEFAULT_CREDENTIALS = {"admin": "pentest2025", "analyst": "analyst2025"}
...
# serveur est la vraie barriere : ici la vraie barriere est
...
MSF_ALLOWED_MODULES = {
    ...
    "auxiliary/scanner/rdp/cve_2019_0708_bluekeep": {
        ...
        "exploit_na_reason": "RCE provoque un BSOD sur la cible — détection seulement",
    },
    "auxiliary/scanner/ssl/openssl_heartbleed": {
        "label": "Détection Heartbleed (CVE-2014-0160)", "category": "detection", "options": ["RHOSTS", "RPORT"],
        "exploit_na_reason": "Fuite d'information seulement, pas de RCE",
    },
    "auxiliary/scanner/http/log4shell_scanner": {
        ...
        "exploit_na_reason": "RCE complète non implémentée — détection seulement",
    },
    ...
    "exploit/unix/ftp/vsftpd_234_backdoor": {
        ...
        "detection_hint": "auxiliary/scanner/ftp/ftp_version",
        "detection_hint_label": "Lancer 'FTP — version' d'abord pour confirmer la bannière vsftpd 2.3.4",
    },
    # NB: aucun scanner MSF ne fait le fingerprint d'UnrealIRCd -> pas de
    # detection_hint (ne pas fabriquer un chemin de module inexistant).
    ...
    "exploit/multi/samba/usermap_script": {
        ...
        "options": ["RHOSTS", "RPORT", "LHOST", "LPORT"],
        "payload": "cmd/unix/reverse",
        "detection_hint": "auxiliary/scanner/smb/smb_version",
        "detection_hint_label": "Lancer 'SMB — version & signature' d'abord pour confirmer la version Samba",
    },
    "exploit/multi/http/apache_mod_cgi_bash_env_exec": {
        "label": "Exploit: Shellshock (CVE-2014-6271)", "category": "exploit",
        "options": ["RHOSTS", "RPORT", "TARGETURI", "LHOST", "LPORT"],
        "payload": "linux/x86/meterpreter/reverse_tcp",
        "detection_hint": "auxiliary/scanner/http/apache_mod_cgi_bash_env",
        "detection_hint_label": "Lancer 'Détection Shellshock' d'abord pour confirmer la vulnérabilité CGI",
    },
    "exploit/windows/smb/ms17_010_eternalblue": {
        "label": "Exploit: EternalBlue (MS17-010)", "category": "exploit",
        "options": ["RHOSTS", "LHOST", "LPORT"],
        "payload": "windows/x64/meterpreter/reverse_tcp",
        "detection_hint": "auxiliary/scanner/smb/smb_ms17_010",
        "detection_hint_label": "Lancer 'Détection EternalBlue (MS17-010)' d'abord pour confirmer la cible vulnérable",
    },
}
...
def _openvas_worker(job_id: str, target: str, user: str, port_list_id: str):
    ...
    try:
        ...
        # Respecte la preference auto-report comme les autres outils (msf, recon,
        # dnsdumpster) : le toggle "openvas" de /api/settings/auto-report etait
        # sans effet ici auparavant. Defaut active -> comportement inchange.
        pentoolbox_report_id = None
        if _auto_report_enabled("openvas"):
...
def login():
    try:
        ...
        if check_password(u, p):
            ...
            # BUG 5 : si le compte se connecte encore avec son mot de passe par
            # defaut, on signale au frontend de forcer un changement (modal non
            # fermable) avant tout acces. Detection sans etat (cf. DEFAULT_CREDENTIALS).
            must_change = (DEFAULT_CREDENTIALS.get(u) == p)
            audit("LOGIN_OK", user=u, details=f"role={role} must_change={must_change}")
...
# hardening Docker) doit elever un sous-process. Plutot que de
...
def nikto_scan():
    ...
    start = time.time()
    # -nointeractive : pas d'invite. -ssl seulement pour les cibles https://
    # (le forcer sur une cible http:// fait echouer la poignee de main TLS).
    # PAS de -nolookup : nikto 2.6 refuse -nolookup quand la cible est un nom
    # d'hote ("ERROR: -nolookup set, but given name") et sort sans scanner ;
    # -nolookup n'est valide qu'avec une IP. La quasi-totalite des cibles
    # etant des noms d'hote, on laisse nikto resoudre normalement.
    ...
    if http_code in ['000', ''] or 'failed' in http_code.lower():
        ...
        ssl_flag = " -ssl" if target.startswith("https://") else ""
        # -maxtime DOIT rester sous le timeout de run_cmd (180s) : sinon run_cmd
        # tue le process avant que nikto n'atteigne son -maxtime, et la sortie
        # (les findings deja trouves) est perdue -> "[!] Timeout apres 180s" vide.
        # 150s : nikto s'arrete proprement et ecrit son rapport, ~30s de marge.
        cmd = f"nikto -h {target} -nointeractive{ssl_flag} -timeout 15 -maxtime 150"
...
def msf_modules():
    ...
    for mid, m in MSF_ALLOWED_MODULES.items():
        out[mid] = {
            ...
            # Suggestion de module de détection à lancer avant l'exploit (None si absent).
            "detection_hint": m.get("detection_hint"),
            "detection_hint_label": m.get("detection_hint_label"),
            # Raison pour laquelle un module de détection n'a pas d'exploit apparié
            # (None sauf pour les 3 scanners détection-seule : BlueKeep/Heartbleed/Log4Shell).
            "exploit_na_reason": m.get("exploit_na_reason"),
        }
    return jsonify(out)
...
def enum4linux_scan():
    ...
    if not _is_safe_scan_target(target):
        return jsonify({"error": "Cible invalide (hôte ou IP attendu, ex: example.com)."}), 400
...
def _save_scan_report(target, vulnerabilities=None, dns_data=None, scan_output="",
    ...
    "Exploitation auto" -> "Generer un rapport" -> cf. doc "Reporting":
