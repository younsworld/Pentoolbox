  ...
  xhr.onload = function() {
    ...
    try {
      ...
      if (data.ok) {
        ...
        // BUG 5 : 1re connexion avec le mot de passe par defaut -> on bloque
        // l'acces derriere une modal non-fermable de changement obligatoire.
        // enterApp() n'est appele qu'apres un changement reussi.
        if (data.must_change_password) {
          showForcedPasswordChange(p);
        } else {
          enterApp();
        }
      } else {
...
// Reveal l'application et lance les chargements post-login. Extrait pour pouvoir
// etre appele soit directement, soit apres un changement de mot de passe force.
function enterApp() {
  document.getElementById('login-screen').style.display = 'none';
  document.getElementById('app').style.display = 'block';
  document.getElementById('user-display-name').textContent = ST.user;
  applyRBAC(ST.role);
  loadSysInfo();
  loadReportCount();
  loadDashboardStats();
...
// BUG 5 — Modal de changement de mot de passe OBLIGATOIRE au 1er login (compte
// encore sur son mot de passe par defaut). Non-fermable : aucun bouton de
// fermeture, pas de clic-backdrop, Echap neutralise. L'app n'est revelee
// (enterApp) qu'apres un changement reussi. Un rechargement de page perd le
// token (en memoire) -> retour a l'ecran de login -> la modal re-apparaitra.
function showForcedPasswordChange(currentPwd) {
  // neutralise Echap tant que la modal est ouverte
  var blockEsc = function(e) { if (e.key === 'Escape') { e.preventDefault(); e.stopPropagation(); } };
  document.addEventListener('keydown', blockEsc, true);

  var ov = document.createElement('div');
  ov.id = 'force-pwd-overlay';
  ov.style.cssText = 'position:fixed;inset:0;z-index:99999;display:flex;align-items:center;'
    + 'justify-content:center;background:rgba(5,8,12,0.92);backdrop-filter:blur(3px);';
  ov.innerHTML =
    '<div style="width:min(440px,92vw);background:#0f141b;border:1px solid #1f2a37;'
    + 'border-radius:12px;padding:28px 26px;box-shadow:0 12px 40px rgba(0,0,0,.6);font-family:inherit;">'
    + '<h2 style="margin:0 0 6px;color:#e6edf3;font-size:18px;">Changement de mot de passe requis</h2>'
    + '<p style="margin:0 0 18px;color:#9aa7b4;font-size:13px;line-height:1.5;">'
    + 'Vous utilisez encore le mot de passe par defaut. Pour des raisons de securite, '
    + 'definissez un nouveau mot de passe avant d\'acceder a la plateforme.</p>'
    + '<label style="display:block;color:#9aa7b4;font-size:12px;margin-bottom:4px;">Nouveau mot de passe</label>'
    + '<input id="fpc-new" type="password" autocomplete="new-password" '
    + 'style="width:100%;box-sizing:border-box;margin-bottom:12px;padding:10px;border-radius:7px;'
    + 'background:#0a0e14;border:1px solid #2a3744;color:#e6edf3;font-size:14px;">'
    + '<label style="display:block;color:#9aa7b4;font-size:12px;margin-bottom:4px;">Confirmer le mot de passe</label>'
    + '<input id="fpc-confirm" type="password" autocomplete="new-password" '
    + 'style="width:100%;box-sizing:border-box;margin-bottom:6px;padding:10px;border-radius:7px;'
    + 'background:#0a0e14;border:1px solid #2a3744;color:#e6edf3;font-size:14px;">'
    + '<div id="fpc-error" style="color:#ff6b6b;font-size:12px;min-height:16px;margin-bottom:12px;"></div>'
    + '<button id="fpc-submit" style="width:100%;padding:11px;border:none;border-radius:7px;cursor:pointer;'
    + 'background:var(--accent,#b6ff3a);color:#0a0e14;font-weight:700;font-size:14px;">'
    + 'Definir le mot de passe</button>'
    + '</div>';
  document.body.appendChild(ov);

  var newI = document.getElementById('fpc-new');
  var confI = document.getElementById('fpc-confirm');
  var errEl = document.getElementById('fpc-error');
  var btn = document.getElementById('fpc-submit');
  newI.focus();

  function submit() {
    var np = newI.value, cp = confI.value;
    errEl.textContent = '';
    if (np.length < 6) { errEl.textContent = 'Minimum 6 caracteres.'; return; }
    if (np !== cp)     { errEl.textContent = 'Les deux mots de passe ne correspondent pas.'; return; }
    if (np === currentPwd) { errEl.textContent = 'Choisissez un mot de passe different du mot de passe par defaut.'; return; }
    btn.disabled = true; btn.textContent = 'Enregistrement...';
    apiFetch('/api/users/' + encodeURIComponent(ST.user) + '/password',
      ...
      .then(function(res){
        if (res.ok && res.d.ok) {
          ...
          ov.remove();
          enterApp();
          if (typeof notify === 'function') notify('Mot de passe modifie.', 'success');
        } else {
          btn.disabled = false; btn.textContent = 'Definir le mot de passe';
          errEl.textContent = (res.d && res.d.error) || 'Echec du changement de mot de passe.';
        }
      })
      .catch(function(){ btn.disabled = false; btn.textContent = 'Definir le mot de passe';
                         errEl.textContent = 'Erreur reseau.'; });
  }
  btn.addEventListener('click', submit);
  confI.addEventListener('keydown', function(e){ if (e.key === 'Enter') submit(); });
  newI.addEventListener('keydown', function(e){ if (e.key === 'Enter') confI.focus(); });
}
...
// Vue "par vulnérabilité" : apparie chaque exploit avec son module de détection
// (via detection_hint) pour rendre la symétrie/asymétrie lisible d'un coup d'œil.
// 100% dérivée de MSF_MODULES (category / is_exploit / detection_hint /
// exploit_na_reason) — aucune donnée n'est dupliquée ici. Chaque cellule
// cliquable pilote le <select> du lanceur via selectMsfModule (le lanceur reste la
// commande canonique ; ce tableau n'est qu'une aide à la navigation).
function renderMsfMatrix() {
  var box = document.getElementById('msf-matrix');
  if (!box) return;
  var ids = Object.keys(MSF_MODULES);
  if (!ids.length) { box.innerHTML = '<span style="color:var(--text2);font-size:12px">Aucun module (RPC injoignable).</span>'; return; }

  var ARROW = '<svg class="icon"><use href="#icon-arrow-right"/></svg>';
  var BAN = '<svg class="icon"><use href="#icon-ban"/></svg>';

  // Cellule cliquable vers un module (grisée si indisponible), ou cellule N/A.
  // Réutilise les classes du thème (.msf-cell -> accent lime + icônes SVG du jeu
  // d'icônes de l'app) plutôt que des styles inline ad hoc.
  function cell(id, reason) {
    if (!id) return '<span class="msf-cell na">' + BAN + 'N/A — ' + escapeHtml(reason || '') + '</span>';
    var m = MSF_MODULES[id];
    if (!m) return '<span class="msf-cell na">' + BAN + escapeHtml(id) + '</span>';
    if (!m.available) return '<span class="msf-cell off">' + ARROW + escapeHtml(m.label) + ' — indisponible</span>';
    return '<a href="#" class="msf-cell" onclick="selectMsfModule(\'' + escapeHtml(id) + '\');return false;">' + ARROW + escapeHtml(m.label) + '</a>';
  }
  function row(vuln, detId, expId, detReason, expReason) {
    return '<tr>' +
      '<td class="msf-vuln">' + escapeHtml(vuln) + '</td>' +
      '<td>' + cell(detId, detReason) + '</td>' +
      '<td>' + cell(expId, expReason) + '</td></tr>';
  }

  // Inverse detection_hint -> {scannerId: exploitId} et collecte les exploits.
  var exploits = ids.filter(function(id){ return MSF_MODULES[id].is_exploit; });
  var pairedScanners = {};   // scannerId utilisé comme moitié "détection" d'une paire
  exploits.forEach(function(eid){
    var h = MSF_MODULES[eid].detection_hint;
    if (h) pairedScanners[h] = eid;
  });

  var rows = [];
  // 1) Paires réelles : exploit AVEC detection_hint.
  exploits.forEach(function(eid){
    var m = MSF_MODULES[eid];
    if (m.detection_hint) rows.push(row(vulnName(m.label), m.detection_hint, eid));
  });
  // 2) Exploit SANS detection_hint (UnrealIRCd) : détection N/A -> NSE.
  exploits.forEach(function(eid){
    var m = MSF_MODULES[eid];
    if (!m.detection_hint) rows.push(row(vulnName(m.label), null, eid, 'utiliser Exploitation Automatisée (NSE)'));
  });
  // 3) Scanners "détection" non appariés (détection seule) : exploit N/A + raison.
  ids.forEach(function(id){
    var m = MSF_MODULES[id];
    if (m.category === 'detection' && !pairedScanners[id]) {
      rows.push(row(vulnName(m.label), id, null, null, m.exploit_na_reason || 'pas d\'exploit exposé'));
    }
  });

  // 4) Scanners de version génériques (non utilisés comme moitié d'une paire) : bande basse.
  var generic = ids.filter(function(id){
    return MSF_MODULES[id].category === 'version' && !pairedScanners[id];
  });
  var genStrip = generic.length ? '<div class="msf-generic">' +
    '<span class="msf-generic-label">Scanners génériques</span>' +
    generic.map(function(id){ return cell(id); }).join('') + '</div>' : '';

  box.innerHTML =
    '<table class="result-table">' +
    '<thead><tr><th>Vulnérabilité</th><th>Détection</th><th>Exploitation</th></tr></thead>' +
    '<tbody>' + rows.join('') + '</tbody></table>' + genStrip;
}

// Extrait un nom de vulnérabilité lisible depuis un label de module
// ("Exploit: vsftpd v2.3.4 Backdoor" -> "vsftpd v2.3.4 Backdoor",
//  "Détection EternalBlue (MS17-010)" -> "EternalBlue (MS17-010)").
function vulnName(label) {
  return (label || '').replace(/^Exploit:\s*/i, '').replace(/^Détection\s+/i, '').trim();
}
...
function onMsfModuleChange() {
  ...
  // Note "vérification recommandée" : pour un exploit doté d'un module de
  // détection MSF, on propose un lien cliquable vers ce scanner. Pour un
  // exploit sans scanner MSF (UnrealIRCd), on affiche une note honnête qui
  // renvoie vers l'Exploitation Automatisée (NSE) plutôt qu'un faux module.
  var hintBox = document.getElementById('msf-hint');
  if (!hintBox) {
    hintBox = document.createElement('div');
    hintBox.id = 'msf-hint';
    hintBox.className = 'msf-hint';
    box.parentNode.insertBefore(hintBox, box.nextSibling);
  }
  hintBox.innerHTML = '';
  var BULB = '<svg class="icon"><use href="#icon-lightbulb"/></svg>';
  if (m.detection_hint) {
    var hm = MSF_MODULES[m.detection_hint];
    var name = hm ? hm.label : m.detection_hint;
    hintBox.innerHTML = BULB + '<span>' + escapeHtml(m.detection_hint_label || 'Vérification recommandée') + ' :</span>' +
      '<a href="#" class="msf-cell" onclick="selectMsfModule(\'' + escapeHtml(m.detection_hint) +
      '\');return false;"><svg class="icon"><use href="#icon-arrow-right"/></svg>' + escapeHtml(name) + '</a>';
  } else if (m.is_exploit) {
    hintBox.innerHTML = BULB + '<span>Aucun scanner MSF disponible — utiliser ' +
      '<b>Exploitation Automatisée (NSE)</b> pour le fingerprint d\'abord.</span>';
  }
}

// Sélectionne un module dans le <select> MSF (utilisé par le lien de la note
// de vérification). Refuse de basculer vers un module de détection indisponible.
function selectMsfModule(id) {
  var sel = document.getElementById('msf-module');
  if (!sel) return;
  var opt = sel.querySelector('option[value="' + id + '"]');
  if (opt && opt.disabled) { notify('Module de détection indisponible (RPC/absent)', 'warn'); return; }
  sel.value = id;
  onMsfModuleChange();
}
