...
echo  [*] DNS : %DNS_FLAGS% pentoolbox:latest
if errorlevel 1 (
    echo  [..] Tentative sans caps reseau...
    docker run -d --name pentoolbox --restart unless-stopped -p 5000:5000 -v "%BUILD_DIR%\reports:/app/reports" %DNS_FLAGS% pentoolbox:latest
    if errorlevel 1 (
